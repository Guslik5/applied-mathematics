...
def left_rectangle_method(f, a, b, n):
    ...
    и разбиением отрезка на n равных частей
    """
...
def simpson_method(f, a, b, n):
    """
    Вычисляет значение определенного интеграла на интервале [a, b] с использованием метода Симпсона.

    f - функция, интеграл которой нужно вычислить
    a, b - границы интервала интегрирования
    n - количество отрезков, на которые нужно разбить интервал [a, b]
    """
    h = (b - a) / n
    s = f(a) + f(b)
...
for h in h_values:
    ...
    # вычисляем значения функций в узлах сетки
    f_values = f(x_grid)
    g_values = g(x_grid)
...
"""a = 0
b = 1
true_int_f = quad(lambda x: x, a, b)[0]
true_int_g = quad(lambda x: x**2, a, b)[0]
n_mas = [100000, 10000, 1000, 100]
simpson_method_mas_x = []
simpson_method_mas_x2 = []
left_rectangle_mas_x = []
left_rectangle_mas_x2 = []
right_rectangle_mas_x = []
right_rectangle_mas_x2 = []
trapezoidal_rule_mas_x = []
trapezoidal_rule_mas_x2 = []
def mse(true_val, num_val):
    return math.sqrt((num_val - true_val)**2)

num_int_f = simpson_method(lambda x: x, a, b, 100000)
mse_f = mse(true_int_f, num_int_f)
simpson_method_mas_x.append(mse_f)
num_int_f = simpson_method(lambda x: x, a, b, 10000)
mse_f = mse(true_int_f, num_int_f)
simpson_method_mas_x.append(mse_f)
num_int_f = simpson_method(lambda x: x, a, b, 1000)
mse_f = mse(true_int_f, num_int_f)
simpson_method_mas_x.append(mse_f)
num_int_f = simpson_method(lambda x: x, a, b, 100)
mse_f = mse(true_int_f, num_int_f)
simpson_method_mas_x.append(mse_f)

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
print(simpson_method_mas_x)
# plot the first graph
ax1.plot(n_mas, simpson_method_mas_x, label='Central difference')
ax1.set_xlabel('Step size, h')
ax1.set_ylabel('RMS error')
ax1.set_title('RMS error for x^2')
ax1.legend()
